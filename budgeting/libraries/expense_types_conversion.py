...
import expense_types_conversion_utils
...
def build(expense_report_dict):
    ...
    expense_type_set = expense_types_conversion_utils.get_expense_type_set(expense_report_dict)
    ...
    for year in sorted_years:
        ...
        for month in sorted_months:
            ...
            for expense_report in expense_report_dict[year][month]:
                ...
                expense_types_dict[year][month][expense_report.get_type()] += expense_amount
                if expense_amount < 0:
                    expense_types_dict[year][month][total_expense] += expense_amount
...
def build_gross_expense_types_dict(expense_report_dict):
    ...
    for year in sorted_years:
        ...
        for month in sorted_months:
            ...
            for expense_report in expense_report_dict[year][month]:
                gross_expenses_dict[year][month][expense_report.get_type()] += float(expense_report.get_amount()) / 100
            month_index += 1
        year_index += 1
